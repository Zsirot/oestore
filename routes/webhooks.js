...
const fulfillOrder = async function (stripeOrderId) {
  try {
    const completedOrder =
      await stripe.checkout.sessions.retrieve(stripeOrderId); //stripeOrderId comes from a successfully completed payment
...
function refreshTrigger() {
  ...
  triggerBlock = false;
  console.log(`trigger reset, triggerBlock is now ${triggerBlock}`);
}
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!triggerBlock) {
      ...
      res.status(200).send();
      triggerBlock = true;
      console.log(`triggerBlock set to ${triggerBlock}, processing webhook...`);
